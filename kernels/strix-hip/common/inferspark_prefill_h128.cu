...
// Inferspark Prefill Attention (HDIM=128) — AMD HIP/WMMA port for gfx1151.
//
// Same WMMA Flash-Attention + shared-memory-staged online softmax approach as
// the validated reference inferspark_prefill_wmma.cu, parameterized for
// head_dim=128. Transforms: mma.sync.m16n8k16 -> wmma_f32_16x16x16_bf16_w32;
// cp.async -> synchronous uint4 smem copies; register softmax -> smem-staged
// softmax with the validated C-frag map S[2*e+(l>>4)][l&15].
//
...
#define K16 16
#define WMMA_K_STEPS (HDIM / K16)        // 8
#define QK_N_TILES   (BC / K16)          // 2
#define PV_K_STEPS   (BC / K16)          // 2
#define PV_N_TILES   ((HDIM / K16) / 2)  // 4
#define TILE_CHUNKS (BR * (HDIM / 8))

__device__ __forceinline__ float sw_exp_h(float x) {
...
#define INFERSPARK_H128_BODY(BLK)                                                      \
    ...
    const unsigned int q_start = q_block * (BLK);                                      \
    if (q_start >= seq_len) return;                                                    \
    const unsigned int q_end = min(q_start + (BLK), seq_len);                          \
    ...
    if (causal) { unsigned int mk = (q_end - 1) / BC; num_kv_blocks = min(num_kv_blocks, mk + 1); } \
    for (unsigned int r = tid; r < BR; r += 128) { smem_ml[r][0] = -1e30f; smem_ml[r][1] = 0.0f; } \
    {                                                                                  \
        ...
        for (unsigned int idx = tid; idx < TILE_CHUNKS; idx += 128) {                  \
            ...
            if (q_row < seq_len) {                                                     \
                *((uint4*)&smem_Q[row][col]) = *((const uint4*)&Q_batch[q_row * q_seq_stride + q_head * head_dim + col]); \
    ...
    for (unsigned int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {            \
        ...
        {                                                                              \
            ...
            for (unsigned int idx = tid; idx < TILE_CHUNKS; idx += 128) {              \
                ...
                if (kv_row < seq_len) {                                                \
                    *((uint4*)&smem_K[row][col]) = *((const uint4*)&K_batch[kv_row * kv_seq_stride + kv_head * head_dim + col]); \
                    *((uint4*)&smem_V[row][col]) = *((const uint4*)&V_batch[kv_row * kv_seq_stride + kv_head * head_dim + col]); \
                } else {                                                               \
        ...
        __syncthreads();                                                               \
        if (tid < BR) {                                                                \
            unsigned int r = tid; unsigned int qr = q_start + r; bool row_valid = (r < q_len); \
            float rmax = -1e30f;                                                       \
            for (unsigned int c = 0; c < BC; c++) {                                    \
                float s = smem_S[r][c] * inv_sqrt_d; unsigned int kpos = kv_start + c; \
                bool masked = (c >= kv_len) || !row_valid;                             \
                if (causal && kpos > qr) masked = true;                                \
                if (causal && sliding_window > 0 && kpos <= qr && (qr - kpos) >= sliding_window) masked = true; \
                if (masked) s = -1e30f; smem_S[r][c] = s; rmax = fmaxf(rmax, s);        \
            }                                                                          \
            float m_old = smem_ml[r][0]; float l_old = smem_ml[r][1];                  \
            float m_new = fmaxf(m_old, rmax); float resc = sw_exp_h(m_old - m_new);    \
            float sum = 0.0f;                                                          \
            for (unsigned int c = 0; c < BC; c++) { float p = sw_exp_h(smem_S[r][c] - m_new); smem_P[r][c] = __float2bfloat16(p); sum += p; } \
            smem_ml[r][0] = m_new; smem_ml[r][1] = l_old * resc + sum; smem_resc[r] = resc; \
        }                                                                              \
        __syncthreads();                                                               \
        { float resc_e[8];                                                             \
