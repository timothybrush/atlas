...
// elements per avarok_cp16) so KV-load latency is hidden behind QK^T compute
...
// 16 FP8 elements (16 bytes) per avarok_cp16, matching the 16-byte transaction
// size. Dequant is deferred to in-register MMA read (AVAROK_ATTN_FP8_SMEM path
...
// HDIM_PAD must itself be 16-aligned for the 16-byte avarok_cp16 and uint4
...
// NVFP4 paths don't define AVAROK_ATTN_FP8_SMEM, so they keep PAD_KV=8.
    ...
    do { \
        ...
        for (unsigned int _i = t; _i < TILE_CHUNKS / 2; _i += (stride)) { \
            ...
            if (_pos < (kv_l)) { \
                ...
                avarok_cp16(&((__nv_fp8_storage_t(*)[HDIM_PAD])(smem))[_row][_col], _base); \
            } else { \
...
// KERNEL_PREAMBLE: empty — AVAROK_ATTN_FP8_SMEM path defers fp8→bf16 dequant
