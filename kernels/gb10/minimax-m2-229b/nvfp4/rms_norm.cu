...
// Block: (min(hidden_size, 1024), 1, 1)
//
// Assumes hidden_size <= blockDim * 4 (each thread handles at most 1 quad),
// and group_size is a multiple of 4.
extern "C" __global__ void gated_rms_norm(
    ...
    unsigned int group_size                     // typically 512 for Nemotron-H
) {
    unsigned int token = blockIdx.x;
    unsigned int tid = threadIdx.x;
    ...
    // ── Per-group reduction using shared memory ──
    // With hidden_size=4096, group_size=512, num_groups=8, blockDim=1024:
    //   128 threads per group (4 warps), each thread handles 1 quad
    // shared: [num_groups][max_warps_per_group] — max 32 warps per block
    __shared__ float group_warp_sums[8][16];  // 8 groups × up to 16 warps/group
    ...
    // Determine which group this warp belongs to (by first thread in warp)
    ...
    // One thread per group computes the total and rms
    if (tid < num_groups) {
        float total = 0.0f;
...
// ─── Ported from kernels/gb10/common/rms_norm.cu ────────────────────
// Shadow-drift repair. This file whole-file-shadows common/rms_norm.cu,
// so every kernel common/ gained after the fork was silently DROPPED
// (try_kernel -> handle 0) even where this model dispatches it.
//
// WEIGHT CONVENTION: common/ applies Qwen3-Next offset-from-1 scaling
// (x * rms * (1 + w)); this model uses the ABSOLUTE formula
// (x * rms * w), matching `rms_norm` at the top of this file. The
// kernels below are verbatim copies with that single expression
// rewritten -- a literal copy would silently mis-normalise every layer.
// ────────────────────────────────────────────────────────────────────
