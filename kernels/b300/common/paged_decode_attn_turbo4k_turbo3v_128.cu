...
// Paged Decode Attention — Asymmetric K=turbo4 (4-bit), V=turbo3 (3-bit) (HDIM=128).
//
// TurboQuant+ both-sides-quantized asym: K compressed to 4-bit Lloyd-Max
// (16-level codebook, 2 vals/byte + per-group FP8 scale) and V to 3-bit
// Lloyd-Max (8-level codebook, 8 vals/3 bytes + per-group FP8 scale). Each
// pool has its own byte layout, byte block stride, and data-section offset.
//
// The Q-side WHT bookend fires unconditionally because K is a turbo dtype
// (attention_forward.rs gates on `k_dtype` which kv_pair() resolves to
// Turbo4 for this variant). The V iWHT on output fires because V is turbo3.
//
...
// Turbo4 K dequant — 4-bit nibbles, 2 values per byte (uint16 load at HDIM=128).
...
// ============================================================================
// Asymmetric Turbo4K + Turbo3V paged decode attention
// ============================================================================
extern "C" __global__ void paged_decode_attn_turbo4k_turbo3v(
    ...
    const unsigned char* __restrict__ V_cache,    // turbo3 byte storage (separate pool)
    ...
    // V-side: 8-level Lloyd-Max codebook (turbo3).
    ...
    // K-pool addressing (turbo4 layout: 4-bit nibbles, 2 values per byte).
    ...
    // V-pool addressing (turbo3 layout: paired threads share 3 bytes for 8 values).
    ...
    // Load Q (BF16, strided) — already WHT'd by attention_forward.rs.
    ...
    while (pos < my_end) {
        ...
        // K and V block bases — distinct pool strides.
        ...
        for (; processed < aligned_count; processed += BC) {
            // Load BC K vectors via turbo4 dequant.
    ...
    // ── Tree-based inter-warp reduction (identical to sym turbo) ──
