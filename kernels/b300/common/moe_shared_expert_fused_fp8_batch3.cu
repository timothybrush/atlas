...
//   y in [3*top_k, 3*top_k+3) -> shared expert  (token = y - 3*top_k)
//
...
__device__ __constant__ float E4M3_LUT_MOE_BATCH3[256] = {
    // Positive (0x00..0x7F)
...
// ── Fused Gate+Up 2x with shared expert — K=3 batch, FP8 variant ──
//
...
extern "C" __global__ void moe_expert_gate_up_shared_fp8_batch3(
    ...
    s_lut[threadIdx.x] = E4M3_LUT_MOE_BATCH3[threadIdx.x];
    s_lut[threadIdx.x + BLOCK_SIZE] = E4M3_LUT_MOE_BATCH3[threadIdx.x + BLOCK_SIZE];
    __syncthreads();
...
// ── Fused SiLU+Down 2x with shared expert — K=3 batch, FP8 variant ──
//
// Grid: (ceil(N/8), 3*(top_k+1), 1)  Block: (128, 1, 1)
extern "C" __global__ void moe_expert_silu_down_shared_fp8_batch3(
...
extern "C" __global__ void moe_weighted_sum_blend_fp8_batch3(
