...
// Paged Decode Attention — Asymmetric K=fp8 (E4M3), V=turbo3 (HDIM=128).
//
// TurboQuant+ asym variant for FP8-attention models: K stored as FP8 with a
// per-tensor `k_scale` (matches the model's quantization, no bf16 K weights
// required), V compressed to 3-bit Lloyd-Max + per-group FP8 scale (saves the
// V-side bandwidth that dominates KV traffic at long context). One CTA per
...
//           (1 byte/elem; pool sized via kv_pair().0 = Fp8)
//   V pool: [num_blocks, block_size_v_layout]
//           where each block holds:
//             [data: block_size * num_kv_heads * head_dim * 3/8 bytes]
...
// FP8 byte vector: each lane covers VEC_BF16 fp8 bytes per K read.
// HDIM=128: VEC_BF16=4 → 4 bytes per lane = exactly one uint32.
// HDIM=256: VEC_BF16=8 → 8 bytes per lane = 2 uint32.
...
// Turbo3 V dequant — paired threads share 3 bytes for 8 values.
// HDIM=128: 4 values per thread per group (even/odd lane split).
...
// ============================================================================
// Asymmetric Fp8K + Turbo3V paged decode attention
// ============================================================================
extern "C" __global__ void paged_decode_attn_fp8k_turbo3v(
    ...
    const unsigned char* __restrict__ V_cache,     // V pool: byte-addressed turbo3
    ...
    // 8-level Lloyd-Max codebook (turbo3) in shared memory as fp16 LUT.
    ...
    // FP8 K offset (bytes) per lane — same per-element index as bf16, but in bytes (1 b/elem)
    const unsigned int vec_offset_fp8 = lane_id * VEC_BF16;

    // V-pool addressing (turbo3 layout)
    ...
    // Paired threads share 3 bytes for 8 values (HDIM=128); HDIM=256 covers
    // its own 3 bytes. Same indexing scheme as paged_decode_attn_turbo3.
#if VEC_BF16 == 8
    const unsigned int v_data_offset_per_thread  = kv_head * v_head_data_bytes  + (lane_id * 3);
    const unsigned int v_scale_offset_per_thread = kv_head * v_head_scale_bytes + (lane_id / 2);
#else
    ...
    while (pos < my_end) {
        ...
        // K-side pointer base (FP8, NHD layout). Pointer arithmetic in bytes
        // since K_cache is `unsigned char*` and FP8 = 1 byte/elem.
        ...
        // V-side pointer base (byte-addressed turbo3, paired threads share 3 bytes).
        ...
        for (; processed < aligned_count; processed += BC) {
            // Load BC K vectors (FP8 contiguous read, VEC_U32_FP8 uint32 per lane).
            ...
            // Compute BC dot products (FP8 dequant + dot with bf16-Q registers).
            float scores[BC];
            #pragma unroll
            for (int b = 0; b < BC; b++) {
                float dot = 0.0f;
            ...
            // Matches the turbo3 decode kernel pattern.
            ...
            for (int b = 0; b < BC; b++) {
                if (exp_factors[b] > TQ_PLUS_SPARSE_V_THRESHOLD) {
                    ...
                    turbo3_dequant_v(vd, vs, e2m1_lut, lane_id, v_vals[b]);
                } else {
                    #pragma unroll
        ...
        for (; processed < batch_count; processed++) {
            ...
            // Sparse V (turbo3 dequant only when exp_new > threshold)
            float v_tmp[VEC_BF16] = {0};
            if (exp_new > TQ_PLUS_SPARSE_V_THRESHOLD) {
                const unsigned char* vd = v_block + p * v_token_data_stride  + v_data_offset_per_thread;
                const unsigned char* vs = v_block + v_data_section_bytes
                                        + p * v_token_scale_stride + v_scale_offset_per_thread;
                turbo3_dequant_v(vd, vs, e2m1_lut, lane_id, v_tmp);
            }
    ...
    // ── Tree-based inter-warp reduction (identical to bf16k_turbo3v / turbo3) ──
