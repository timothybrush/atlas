...
// Paged Decode Attention — Asymmetric K=bf16, V=turbo2 (HDIM=256).
//
// TurboQuant+ safer-asym variant: K kept at full BF16 precision (preserves
// attention-score fidelity), V compressed to 2-bit Lloyd-Max + per-group FP8
// scale (6.4x compression on V — maximum bandwidth savings at long context,
// at a quality cost mitigated by the canonical two-sided WHT rotation done
// on the write path). One CTA per (q_head, seq) pair, 8 warps, BC=4 batched
// positions, sparse V dequant on both batched and remainder paths.
//
...
//             [data: block_size * num_kv_heads * head_dim / 4 bytes  (4 idx/byte)]
...
// HDIM=256: each thread reads VEC_BF16=4 values from 1 byte.
...
// ============================================================================
// Asymmetric Bf16K + Turbo2V paged decode attention
// ============================================================================
extern "C" __global__ void paged_decode_attn_bf16k_turbo2v(
    ...
    // Each thread covers VEC_BF16=4 values = 1 byte (HDIM=256) or 8 values = 2 bytes (HDIM=256).
