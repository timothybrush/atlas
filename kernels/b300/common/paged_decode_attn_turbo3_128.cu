...
extern "C" __global__ void paged_decode_attn_turbo3(
    ...
    const unsigned int kv_data_offset = kv_head * head_data_bytes + ((lane_id / 2) * 3)  /* Turbo3: paired threads share 3 bytes for 8 values */;
    const unsigned int kv_scale_offset = kv_head * head_scale_bytes + (lane_id / 4);  // Turbo3: 4 threads per group (2 pairs × 8 values = 16)
