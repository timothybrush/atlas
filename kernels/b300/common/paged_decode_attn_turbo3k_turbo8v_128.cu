...
// Paged Decode Attention — Asymmetric K=turbo3 (3-bit), V=turbo8 (FP8) (HDIM=128).
//
// TurboQuant+ asymmetric: K compressed to 3-bit Lloyd-Max (8-level codebook,
// paired threads share 3 bytes for 8 values + per-group FP8 scale) and V
// kept at FP8 E4M3 (1 byte/elem) + per-group BF16 scale.
//
...
// Turbo3 K dequant — paired threads share 3 bytes for 8 values.
__device__ __forceinline__ void turbo3_dequant_k(
...
// ============================================================================
// Asymmetric Turbo3K + Turbo8V paged decode attention
// ============================================================================
extern "C" __global__ void paged_decode_attn_turbo3k_turbo8v(
    const __nv_bfloat16* __restrict__ Q,
    const unsigned char* __restrict__ K_cache,    // turbo3 byte storage
    ...
    // K-side: 8-level Lloyd-Max codebook (turbo3).
    __shared__ __half k_lut[8];
    if (tid < 8) {
    ...
    // K-pool addressing (turbo3 layout: paired threads share 3 bytes/8 vals).
    const unsigned int k_head_data_bytes  = head_dim * 3 / 8;
    ...
    const unsigned int k_data_offset_per_thread  = kv_head * k_head_data_bytes  + ((lane_id / 2) * 3);
    const unsigned int k_scale_offset_per_thread = kv_head * k_head_scale_bytes + (lane_id / 4);
    ...
    while (pos < my_end) {
        ...
        for (; processed < aligned_count; processed += BC) {
            ...
            for (int b = 0; b < BC; b++) {
                ...
                turbo3_dequant_k(kd, ks, k_lut, lane_id, k_vals[b]);
            }
        ...
        for (; processed < batch_count; processed++) {
            ...
            turbo3_dequant_k(kd, ks, k_lut, lane_id, k_tmp);
