...
`tests/test_gate_results.py` and `tests/test_gate_results_orchestrator.py` (host-only). Run order for `/avarok-release gate`:
```bash
AVAROK_IMAGE=<tag> python3 tests/run_all_models.py               # boots + probes matrix -> *.json + _manifest.json
