...
2. **Sync (fast-forward only):** advance `origin/main` to `monumental/main`. If it
   ...
   not an auto-merge.
3. **Build + verify:** `/avarok-release build *` → `/avarok-release verify`. A sync
...
AVAROK_SKIP_BUILD=1 CUDARC_CUDA_VERSION=13000 cargo test --workspace --locked
...
`/avarok-release verify` (the serve matrix) proves the image serves. Keep the two
...
| **Image provenance.** `release.sh`/`release.yml` pass `--build-arg AVAROK_GIT_SHA` but the Dockerfile had no `ARG`, so it was dropped. | `ARG`+`LABEL org.opencontainers.image.revision` added to `docker/gb10/Dockerfile`. (Applied.) |
