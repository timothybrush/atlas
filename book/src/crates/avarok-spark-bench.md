...
This is a **client-side** harness. It does not link against `spark-runtime` or `avarok-kernels` — it just speaks OpenAI-compatible HTTP to a server on `localhost:8888` (or wherever `AVAROK_BENCH_URL` points).
...
2. **Same surface as real clients.** If the HTTP stack has a latency bug, `avarok-spark-bench` sees it. If the tokenizer is slow on some prompt shape, it shows up.
...
The job of `avarok-spark-bench` is to produce **end-to-end**, **apples-to-apples** numbers that survive the tokenizer, the scheduler, the HTTP layer, and the kernel set. When the README says "131 tok/s on Qwen3.5-35B", that number came from here.
