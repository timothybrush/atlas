...
Usage:
    AVAROK_NEMO_DUMP=<dir> ./serve_longcat_tui.sh          # arm under test
...
path = os.path.join(DUMP, 'avarok_logits.bin')
...
if not os.path.exists(path):
    print(f'no dump at {path} — is AVAROK_NEMO_DUMP set on the SERVER process?')
    sys.exit(1)
...
print(f'avarok top-1 id     : {int(o_got[0])}  (p at ref top-1 = {p_got_at_ref_top1:.4f})')
print(f'top-1 agree        : {"YES" if top1 else "NO"}')
for k in (5, 10, 20, 100):
    print(f'top-{k:<3d} overlap    : {ov[k]}/{k}')
print(f'logit cosine       : {cos:.6f}')
print(f'KL(ref||avarok)     : {kl:.6f} nats')
...
json.dump({'label': LABEL, 'top1_agree': top1, 'overlap': ov, 'cos': cos,
           ...
           'ref_top1_id': int(o_ref[0]), 'avarok_top1_id': int(o_got[0]),
