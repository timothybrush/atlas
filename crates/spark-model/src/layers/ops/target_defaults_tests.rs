...
//! `avarok-kernels/tests/target_defaults.rs`, which parses the real files with
//! the real build-script parser.

use super::*;
use avarok_kernels::TargetDefaults;
...
/// in `avarok-kernels/tests` pins exactly that split.
...
/// The head band is 16 — see `avarok-kernels/tests/target_defaults.rs`.
...
/// POSITIVE, and the deliverable: an H100 serve with NO `AVAROK_*` set
/// resolves to the batched GDN recurrence, which is the line that used to be
/// `AVAROK_SSM_BATCHED_RECURRENT=1` in an external launch script.
#[test]
...
/// `gemm_quant.rs`, and `avarok-kernels/build_defaults.rs::baseline` repeats it
/// as a literal because avarok-kernels sits below spark-model and cannot name
...
fn a_declared_on_lever_can_be_turned_off_by_the_environment() {
    for off in ["0", "false", "off", "no", "OFF", " 0 "] {
        let l = with(&HOPPER, &[("AVAROK_SSM_BATCHED_RECURRENT", off)]);
...
fn a_declared_off_lever_is_still_armed_by_the_bare_one() {
    let l = with(&GB10, &[("AVAROK_SSM_BATCHED_RECURRENT", "1")]);
    assert!(l.ssm_batched_recurrent.value);
    assert!(l.ssm_batched_recurrent.from_env());
}

/// ⚠️ THE POLARITY CHANGE. `AVAROK_GDN_PREFILL_TC` was PRESENCE-gated, so
...
fn the_tensor_core_prefill_spine_reads_zero_as_off_not_as_present() {
    for off in ["0", "false", "off", "no", "OFF", " 0 "] {
        let l = with(&GB10, &[("AVAROK_GDN_PREFILL_TC", off)]);
        assert!(
            !l.gdn_prefill_tc.value,
    ...
    let on = with(&GB10, &[("AVAROK_GDN_PREFILL_TC", "1")]);
...
fn the_legacy_kill_switch_still_forces_the_lever_off() {
    for value in ["1", "0", ""] {
        let l = with(&HOPPER, &[("AVAROK_NO_DECODE_SPLIT_SILU", value)]);
        assert!(
            !l.decode_split_silu.value,
            "AVAROK_NO_DECODE_SPLIT_SILU={value:?} is PRESENCE-gated and must \
...
fn the_summary_line_names_every_lever_and_flags_the_environment() {
    let line = format_levers(&with(&HOPPER, &[("AVAROK_LM_HEAD_BATCHM_MAX", "12")]));
...
/// `AVAROK_ATTN_DECODE_SPLITK=0` resolves to `Pinned(1)` — one split IS no
...
fn the_split_k_policy_resolves_and_reports_like_every_other_lever() {
    ...
    let off = with(&HOPPER, &[("AVAROK_ATTN_DECODE_SPLITK", "0")]);
    ...
    let on = with(&GB10, &[("AVAROK_ATTN_DECODE_SPLITK", "auto")]);
    ...
    // receipt for it.
    let typo = with(&HOPPER, &[("AVAROK_ATTN_DECODE_SPLITK", "atuo")]);
...
fn the_process_resolution_reads_this_binarys_declaration() {
    ...
    assert_eq!(
        resolved().hw,
        avarok_kernels::TARGET_DEFAULTS.hw,
