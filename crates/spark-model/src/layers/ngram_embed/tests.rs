...
/// 63 GB). Data dir comes from `AVAROK_NGRAM_TEST_DATA`, generated by
/// `bench/ngram_ref/make_gpu_testdata.py`. GPU test: `#[ignore]` per
/// repo convention (CI is CPU-only); run with
///   AVAROK_NGRAM_TEST_DATA=/tank/avarok-testdata/ngram_longcat_lite \
///   cargo test -p spark-model --release ngram_gpu -- --ignored
#[test]
#[ignore]
fn ngram_gpu_matches_golden() {
    let dir = std::env::var("AVAROK_NGRAM_TEST_DATA")
        .expect("set AVAROK_NGRAM_TEST_DATA (see make_gpu_testdata.py)");
...
fn cached_table_matches_resident_table() {
    ...
    let tmp = std::env::temp_dir().join(format!("avarok_ngram_cache_{}.bin", std::process::id()));
