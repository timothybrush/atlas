...
//! The `Qwen3SsmLayer` struct itself.
//!
//! Split out of `mod.rs` under the 500-LoC cap: the struct and its field
//! documentation are 417 of that file's 506 lines, and the 2026-09-04
//! batched-verify line pushed it over. Moving the declaration leaves `mod.rs`
//! as what it should be -- the module's table of contents -- and is an exact
//! piecewise copy of the declaration -- no field, attribute or doc comment is
//! changed -- with ONE deliberate adjustment: fields that carried no
//! visibility modifier are now `pub(super)`. That is not a widening. A private
//! field in `mod.rs` was visible to every descendant of `qwen3_ssm`, which is
//! where all the sibling impl files live; private in a CHILD module it would
...
pub struct Qwen3SsmLayer {
    ...
    pub(super) w4a16_gemm_t_k: KernelHandle, // Transposed B layout [K/2, N] — K_STEP_T=32
    pub(super) w4a16_gemm_t_k64_k: KernelHandle, // K64 variant: K_STEP_T=64, halves outer loop
    ...
    pub(super) w4a16_gemm_t_m128_k: KernelHandle, // M128 variant: 2 M-chunks per CTA, halves B re-reads
    pub(super) w4a16_gemm_t_m128_v2_k: KernelHandle, // M128 8-warp pipelined (fast at small M; the FFN's kernel)
    ...
    pub(super) fp8_gemm_t_m128_k: KernelHandle, // M128: halves B re-reads for out_proj at ISL > 128
