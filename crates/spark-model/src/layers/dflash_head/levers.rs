...
///
/// Plain `Copy` data. Every field is a pure function of one `AVAROK_*`
...
pub struct DFlashLevers {
    ...
    ///
    /// ★ Presence, not truth — `AVAROK_DFLASH_BLOCK_DUMP=0` suppresses graph
    ...
    /// `AVAROK_DFLASH_BLOCK_DUMP=<invalid utf-8>` used to leave capture ON.
    ...
    // ── One-shot dumps ──
    /// `AVAROK_DFLASH_DEBUG_DUMP=1` — first 10 BF16 floats of each key
    /// intermediate, for element-wise comparison against a Python reference.
    pub debug_dump: bool,
    /// `AVAROK_DFLASH_DEBUG_DUMP_FULL=1` — full tensors, not the first 10.
    pub debug_dump_full: bool,
    /// `AVAROK_DFLASH_LOG_DRAFTS=1` — log the γ drafts each propose returns.
    pub log_drafts: bool,
    /// `AVAROK_DFLASH_BLOCK_DUMP=1` — per-layer `.bin` dumps of the block
    /// inputs and every layer's output.
    pub block_dump: bool,
    /// `AVAROK_DFLASH_BLOCK_DUMP_AT_POS=<n>` — arm the block dump only at
    ...
    /// config (#649); `AVAROK_DFLASH_OPTION_B=0` is the kill switch.
    ///
    ...
    pub option_b: bool,
    /// `AVAROK_DFLASH_OPTION_B_NO_CTX=1` — force `ctx_count = 0` in the layer
    ...
    /// the components; `AVAROK_DFLASH2=0` disables.
    pub dflash2: bool,
    /// `AVAROK_DFLASH_BATCH_PROPOSE=<width>` caps the cross-sequence batch.
    ...
    /// `AVAROK_DFLASH_DRAFT_CAP=<n>` — submit at most n drafts per propose.
    /// `None` means the head's own γ.
    pub draft_cap: Option<usize>,

    // ── Propose-path diagnostics ──
    /// `AVAROK_DFLASH_VERIFY_TRACE=1` — log all γ drafts BEFORE the cap, so
    /// an echo at position 0 can be told from an echo on every noise row.
    pub verify_trace: bool,
    /// `AVAROK_DFLASH_PRECOMPUTE_DUMP=1` — one-shot dump of the fused ctx K/V
    /// GEMM inputs and outputs.
    pub precompute_dump: bool,
    /// `AVAROK_DFLASH_CTX_PARITY_DUMP=1` — one-shot dump of the accumulated
    /// ctx hidden rows for a PyTorch parity diff.
    pub ctx_parity_dump: bool,
    /// `AVAROK_DFLASH_DEBUG_NO_DECODE_APPEND=1` — skip the post-decode ctx
    /// append entirely.
    pub no_decode_append: bool,
    /// `AVAROK_DFLASH_DEBUG_FULL_PRECOMPUTE=1` — recompute the whole ctx
    ...
    /// `AVAROK_DFLASH_CTXLEN_PROBE=1` — assert `ctx_positions` is strictly
    ...
    /// head; `AVAROK_DSPARK_MARKOV=0` degrades to the batched argmax path
    /// bit-for-bit.
    pub dspark_markov: bool,
    /// `AVAROK_DSPARK_CONF_TAU=<t>` — sigmoid-space acceptance threshold for
    /// the confidence head. `0.0` (unset) disables the head entirely,
    /// matching the reference's `threshold <= 0.0 -> full block`.
    pub conf_tau: f32,
    /// `AVAROK_DSPARK_SHIFT=1|0` forces the SpecForge shifted-row convention
    ...
    /// `AVAROK_DSPARK_ANCHOR_BIAS=0` exempts it. Confidence truncation reads
    /// this too — rows without the chain never write their confidence slot.
    pub dspark_anchor_bias: bool,
    /// `AVAROK_DSPARK_CONF_TRACE=1` — log the confidence logits and sigmoids.
...
const GRAPH_SUPPRESSING_DIAGNOSTICS: [&str; 11] = [
    "AVAROK_DFLASH_PROPOSE_NO_GRAPH",
    "AVAROK_DFLASH_DEBUG_DUMP_FULL",
    "AVAROK_DFLASH_OPTION_B_DIAG",
    "AVAROK_DFLASH_PRECOMPUTE_DUMP",
    "AVAROK_DFLASH_VERIFY_TRACE",
    "AVAROK_DFLASH_LOG_DRAFTS",
    "AVAROK_DFLASH_DEBUG_FORCE_PATTERN",
    "AVAROK_DFLASH_DEBUG_FORCE_NOISE_PATTERN",
    "AVAROK_DFLASH_DEBUG_CTX_OFF",
    "AVAROK_DFLASH_DEBUG_CTX_USED",
    "AVAROK_DFLASH_BLOCK_DUMP",
];
...
fn from_values(
    ...
    DFlashLevers {
        ...
        debug_dump: opt_in(value("AVAROK_DFLASH_DEBUG_DUMP").as_deref()),
        debug_dump_full: opt_in(value("AVAROK_DFLASH_DEBUG_DUMP_FULL").as_deref()),
        log_drafts: opt_in(value("AVAROK_DFLASH_LOG_DRAFTS").as_deref()),
        block_dump: opt_in(value("AVAROK_DFLASH_BLOCK_DUMP").as_deref()),
        block_dump_at_pos: parsed(value("AVAROK_DFLASH_BLOCK_DUMP_AT_POS").as_deref()).unwrap_or(0),
        option_b_diag: opt_in(value("AVAROK_DFLASH_OPTION_B_DIAG").as_deref()),

        force_pattern: opt_in(value("AVAROK_DFLASH_DEBUG_FORCE_PATTERN").as_deref()),
        force_noise_pattern: opt_in(value("AVAROK_DFLASH_DEBUG_FORCE_NOISE_PATTERN").as_deref()),
        force_no_ctx: opt_in(value("AVAROK_DFLASH_DEBUG_CTX_OFF").as_deref()),
        force_ctx_used: parsed(value("AVAROK_DFLASH_DEBUG_CTX_USED").as_deref()),

        precompute: opt_in(value("AVAROK_DFLASH_PRECOMPUTE").as_deref()),
        precompute_commit: opt_in(value("AVAROK_DFLASH_PRECOMPUTE_COMMIT").as_deref()),

        option_b: super::option_b_from(value("AVAROK_DFLASH_OPTION_B").as_deref()),
        option_b_no_ctx: opt_in(value("AVAROK_DFLASH_OPTION_B_NO_CTX").as_deref()),
        dflash2: value("AVAROK_DFLASH2").as_deref() != Some("0"),
        batch_propose_width: parsed(value("AVAROK_DFLASH_BATCH_PROPOSE").as_deref())
            .unwrap_or(usize::MAX),
        draft_cap: parsed(value("AVAROK_DFLASH_DRAFT_CAP").as_deref()),

        verify_trace: opt_in(value("AVAROK_DFLASH_VERIFY_TRACE").as_deref()),
        precompute_dump: opt_in(value("AVAROK_DFLASH_PRECOMPUTE_DUMP").as_deref()),
        ctx_parity_dump: opt_in(value("AVAROK_DFLASH_CTX_PARITY_DUMP").as_deref()),
        no_decode_append: opt_in(value("AVAROK_DFLASH_DEBUG_NO_DECODE_APPEND").as_deref()),
        full_precompute: opt_in(value("AVAROK_DFLASH_DEBUG_FULL_PRECOMPUTE").as_deref()),
        ctxlen_probe: opt_in(value("AVAROK_DFLASH_CTXLEN_PROBE").as_deref()),

        dspark_markov: value("AVAROK_DSPARK_MARKOV").as_deref() != Some("0"),
        conf_tau: parsed(value("AVAROK_DSPARK_CONF_TAU").as_deref()).unwrap_or(0.0),

        propose_warmup_n: parsed(value("AVAROK_DFLASH_PROPOSE_WARMUP_N").as_deref()).unwrap_or(2),

        dspark_shift: match value("AVAROK_DSPARK_SHIFT").as_deref() {
            Some("1") => Some(true),
            Some("0") => Some(false),
            _ => None,
        },
        dspark_anchor_bias: value("AVAROK_DSPARK_ANCHOR_BIAS").as_deref() != Some("0"),
        dspark_conf_trace: opt_in(value("AVAROK_DSPARK_CONF_TRACE").as_deref()),
    }
}
...
impl DFlashLevers {
    ...
    /// What a head resolves to with no `AVAROK_*` set: every diagnostic off,
