...
fn fp() -> ModelFingerprint {
    let cfg = avarok_core::config::ModelConfig::qwen3_next_80b_nvfp4();
...
fn decode_tier_defaults_to_host_ram_non_dropping() {
    assert!(
        std::env::var_os("AVAROK_SSM_DECODE_TIER").is_none(),
        "unset AVAROK_SSM_DECODE_TIER: this check owns the default branch"
    );
...
fn disk_cap_defaults_to_unbounded() {
    // Default-OFF guard for AVAROK_SSM_TIER_DISK_GB: unset ⇒ 0 ⇒ the Marconi
    // arms construct exactly the pre-cap unbounded store.
    for var in [
        "AVAROK_SSM_TIER_DISK_GB",
        "AVAROK_SSM_RDMA_TIER",
        "AVAROK_SSM_TIER_UNIFIED",
        "AVAROK_SSM_SWAP",
    ] {
        assert!(
