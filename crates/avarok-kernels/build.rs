...
struct SamplingCat {
    ...
    // DRY sampler params (see SamplingCategory in avarok-kernels/src/lib.rs
...
///   `AVAROK_SKIP_BUILD=1`; emitting them only on the compiling path would
...
///   `AVAROK_KERNEL_SET_HASH` already closes. A second one would need its own
///   copy of that argument.
fn target_defaults_literal(workspace_root: &std::path::Path) -> String {
    let hw = env::var("AVAROK_TARGET_HW").unwrap_or_else(|_| build_diagnose::DEFAULT_HW.into());
...
fn main() {
    ...
    let workspace_root_owned = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
        .parent()
        .and_then(|p| p.parent())
        .expect("crates/avarok-kernels is two levels below the workspace root")
        .to_path_buf();
    ...
    // prior `AVAROK_TARGET_MODEL=*` build).
    println!("cargo:rerun-if-env-changed=AVAROK_TARGET_HW");
    println!("cargo:rerun-if-env-changed=AVAROK_TARGET_MODEL");
    println!("cargo:rerun-if-env-changed=AVAROK_TARGET_QUANT");
    // AVAROK_EXTRA_NVCC_FLAGS — global nvcc-flag override read by
    // `build_target::NvidiaTarget::compile`. Used for kernel bisection
    // tests (e.g. `-DAVAROK_FAST_SOFTMAX_EXP=1` to flip the softmax
    // polynomial vs `__expf` `#ifdef` choice).
    println!("cargo:rerun-if-env-changed=AVAROK_EXTRA_NVCC_FLAGS");
    ...
    // `kernels/metal/` and let `AVAROK_TARGET_HW=metal` drive a real build.
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    let hw_explicit = env::var("AVAROK_TARGET_HW").is_ok();
    let auto_skip_macos = target_os == "macos" && !hw_explicit;
    let skip_env = matches!(
        env::var("AVAROK_SKIP_BUILD")
            .or_else(|_| env::var("ATLAS_SKIP_BUILD"))
            .as_deref(),
        Ok("1") | Ok("true")
    );
    if skip_env || auto_skip_macos {
        ...
        println!(
            "cargo:rustc-env=AVAROK_KERNEL_SET_HASH={}",
            content_hash(&stub)
        );
        println!("cargo:rustc-env=AVAROK_PTX_DIR={}", out_dir.display());
        ...
        println!("cargo:rustc-env=AVAROK_TARGET_CLOSURES={{}}");
        return;
    }
    ...
    assert!(
        ...
        build_diagnose::no_targets(
            ...
            &env::var("AVAROK_TARGET_HW").unwrap_or_else(|_| build_diagnose::DEFAULT_HW.into()),
            &env::var("AVAROK_TARGET_MODEL").unwrap_or_else(|_| "*".into()),
            &env::var("AVAROK_TARGET_QUANT").unwrap_or_else(|_| "nvfp4".into()),
        )
    );
    ...
    let hw_dir = workspace_root
        .join("kernels")
        .join(env::var("AVAROK_TARGET_HW").unwrap_or_else(|_| build_diagnose::DEFAULT_HW.into()));
    ...
    // as `cargo:rustc-env=AVAROK_HW_FORCE_BR32`, for an `option_env!` reader in
    ...
    //   (a) Stage the CUDA→HIP compat-header dir (avarok-kernels/hip/compat) and
    //       export AVAROK_HIP_COMPAT_INCLUDE — HipTarget::compile force-includes
    ...
    if is_hip {
        ...
        assert!(
            ...
            "HIP compat headers missing at {} — expected the staged \
             avarok-kernels/hip/compat dir (cuda_runtime.h, cuda_bf16.h, cuda_fp8.h).",
            compat_dir.display()
        );
        println!(
            "cargo:rustc-env=AVAROK_HIP_COMPAT_INCLUDE={}",
            compat_dir.display()
        );
        ...
        unsafe {
            std::env::set_var("AVAROK_HIP_COMPAT_INCLUDE", &compat_dir);
        }
    ...
    for (idx, target) in targets.iter().enumerate() {
        ...
        if !reportable.is_empty() {
            println!(
                "cargo:warning=avarok-kernels: ({}, {}) drops {} kernel(s) by shadowing common/: {}. \
    ...
    if total > 0 {
        println!(
            "cargo:warning=avarok-kernels: dedup+parallel: {nvcc_invocations}/{total} unique nvcc \
    ...
    // Force avarok-kernels lib recompilation whenever the generated kernel set
    ...
    // campaign.
    println!(
        "cargo:rustc-env=AVAROK_TARGET_CLOSURES={}",
...
fn closure_attestation(
    ...
    for target in targets {
        ...
        let inputs = avarok_closure::ClosureInputs {
            sources,
            configs,
        ...
        let Ok(closure) = avarok_closure::hash_with_report(workspace_root, &inputs) else {
            continue;
        };
...
/// Used to force avarok-kernels recompilation when the generated kernel set
...
fn build_hip_shim(manifest_dir: &std::path::Path, out_dir: &std::path::Path) {
    ...
    let hipcc = std::env::var("AVAROK_HIPCC").unwrap_or_else(|_| "/opt/rocm/bin/hipcc".into());
    ...
    //   libcublasLt.so — cuBLASLt, stub (opt-in AVAROK_CUBLAS_GEMM path only)
    ...
    println!(
        "cargo:warning=avarok-kernels: built HIP shims (libcuda/libcudart/libcublasLt) in {}",
        out_dir.display()
    );
}
...
fn build_hip_shim_windows(manifest_dir: &std::path::Path, out_dir: &std::path::Path) {
    let hipcc = std::env::var("AVAROK_HIPCC")
        .expect("AVAROK_HIPCC must point at the Windows HIP SDK hipcc for the hip target");
    let hip_path =
    ...
    let def = out_dir.join("avarok_hip_cuda.def");
    ...
    if !staged_runtime {
        println!(
            "cargo:warning=avarok-kernels: no amdhip64*.dll in {}\\bin to bundle — it is an AMD-driver component, present on real AMD Windows hosts at runtime.",
            hip_root.display()
        );
    }
    ...
    println!(
        "cargo:rustc-env=AVAROK_HIP_RUNTIME_DIR={}",
        out_dir.display()
    );
    println!(
        "cargo:warning=avarok-kernels: built Windows HIP runtime shim (cuda.dll/nvcuda.dll + import libs, {} exports) at {}",
        exports.len(),
        out_dir.display()
    );
}
...
fn resolve_targets(workspace_root: &std::path::Path) -> Vec<Target> {
    ...
    let hw = env::var("AVAROK_TARGET_HW").unwrap_or_else(|_| build_diagnose::DEFAULT_HW.into());
    let model_spec = env::var("AVAROK_TARGET_MODEL").unwrap_or_else(|_| "*".into());
    let quant_spec = env::var("AVAROK_TARGET_QUANT").unwrap_or_else(|_| "nvfp4".into());
    ...
    // `-DAVAROK_NO_WARP_BLOCKSCALE_MMA` here because neither ISA has the
...
///
/// Runtime resolution (`avarok_kernels::resolve`) breaks such a collision by
