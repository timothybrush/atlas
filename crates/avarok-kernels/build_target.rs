...
impl ComputeTarget for NvidiaTarget {
    ...
    fn compile(
        ...
        // tolerated. On by default; set AVAROK_KERNEL_NO_STRICT=1 to disable
        // locally (only the literal "1" disables, so AVAROK_KERNEL_NO_STRICT=0
        ...
        if std::env::var("AVAROK_KERNEL_NO_STRICT").as_deref() != Ok("1") {
...
///
/// Pairs with the `libcuda.so` → HIP shim (avarok-kernels/hip/libcuda_hip_shim.cpp):
...
/// exports its path via `AVAROK_HIP_COMPAT_INCLUDE`; the per-source mask-widen
...
impl ComputeTarget for HipTarget {
    ...
    fn compile(
        ...
        let compat = std::env::var("AVAROK_HIP_COMPAT_INCLUDE").map_err(|_| {
            "AVAROK_HIP_COMPAT_INCLUDE not set — build.rs must stage the CUDA→HIP \
        ...
        if cfg!(windows) {
            ...
            args.push("avarok_hip_win_shims.h".into());
        }
...
fn find_hipcc() -> PathBuf {
    if let Ok(p) = std::env::var("AVAROK_HIPCC") {
    ...
    panic!("hipcc not found — install ROCm or set AVAROK_HIPCC to its path.");
}
    ...
    match vendor.unwrap_or("nvidia") {
        ...
        other => panic!(
            "Unsupported compute vendor '{other}'. Supported: nvidia, apple, amd, hip.\n\
             ...
             in avarok-kernels/build_target.rs and avarok-core/src/compute.rs."
        ),
    }
}
