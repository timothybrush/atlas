...
//! `AVAROK_SKIP_BUILD=1`, where `build.rs` returns before the detector ever
...
"#,
    ...
    std::fs::write(
        common.join("attn.cu"),
        "#define KERNEL_NAME avarok_attn\n#include \"compute.cuh\"\n",
    )
    .unwrap();
    // The fork kept the primary kernel and never picked up the BR=64 variant.
    std::fs::write(
        model.join("attn.cu"),
        "extern \"C\" __global__ void avarok_attn(const float* x) {}\n",
    )
    .unwrap();
...
/// binary — but CI builds with `AVAROK_SKIP_BUILD=1`, which returns long before
