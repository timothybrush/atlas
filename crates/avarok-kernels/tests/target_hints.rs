...
//! `avarok_core::arch::target_hint` against the tree it claims to describe.
//!
//! `target_hint` answers "which `AVAROK_TARGET_HW=` should this operator have
//! built" for a device compute capability, and it is a hand-written `match`.
//! It lives in avarok-core because a shipped binary needs it and avarok-core is
            ...
            .unwrap_or_else(|| {
                panic!(
                    "kernels/{dir}/HARDWARE.toml is an NVIDIA target with no \
                     compute_capability; avarok_core::arch::target_hint cannot be \
                     held to a value that is not declared"
                )
            });
        let cc = parse_cc(cc_text, &dir);
        assert_eq!(
            avarok_core::arch::target_hint(cc),
            Some(dir.as_str()),
            "kernels/{dir}/HARDWARE.toml declares compute_capability {cc_text:?}, \
             but avarok_core::arch::target_hint({cc:?}) does not name {dir:?} — \
             add the arm in crates/avarok-core/src/arch.rs"
        );
        checked.push(dir);
    }
    assert!(
...
fn every_target_hint_names_a_hardware_set_that_exists() {
    ...
    for cc in [(9, 0), (10, 0), (12, 1)] {
        ...
        assert!(
            ...
            "target_hint({cc:?}) says to build AVAROK_TARGET_HW={hw}, but \
