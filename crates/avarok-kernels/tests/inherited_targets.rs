...
//!
//! `cargo test` runs GPU-free with `AVAROK_SKIP_BUILD=1`, where `build.rs`
...
//! MODEL.tomls, and what `build.rs` would resolve. The `AVAROK_NO_WARP_
...
/// reads it and holds `avarok_core::arch::target_hint` to it, so the value has
/// to be right.
#[test]
...
/// expand `AVAROK_TARGET_MODEL=*`: a subdirectory that has a MODEL.toml.
...
/// reaches it: `AVAROK_SKIP_BUILD=1` returns from `main()` before target
