...
/// `avarok_core::device::sm121::NUM_SMS = 48` — a constant named after one card
...
fn every_target_declares_the_sm_count_of_its_own_part() {
    ...
    // because this runs on the AVAROK_SKIP_BUILD path too.
...
/// panicking: the generator runs on the `AVAROK_SKIP_BUILD` path, where
...
/// field would surface as an unrelated error in `avarok-kernels` rather than
/// here.
#[test]
...
fn the_baked_constant_matches_its_own_hardware_tree() {
    let baked = avarok_kernels::TARGET_DEFAULTS;
    // `AVAROK_SKIP_BUILD` (every CPU gate) with no `AVAROK_TARGET_HW` bakes the
