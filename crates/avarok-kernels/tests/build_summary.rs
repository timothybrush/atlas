...
//! recovered `196` from `target/release/build/avarok-kernels-*/output`, then
...
fn the_summary_line_is_pinned_whole() {
    assert_eq!(
        ...
        "avarok-kernels: 199 kernels (hopper, qwen3.8-27b, nvfp4), \
...
fn zero_counts_still_print() {
    ...
    assert_eq!(
        line,
        "avarok-kernels: 158 kernels (gb10, qwen3.6-27b, nvfp4), \
...
fn each_target_is_identified_by_its_own_triple() {
    ...
    for line in &lines {
        assert!(line.starts_with("avarok-kernels: "), "{line}");
    }
}
