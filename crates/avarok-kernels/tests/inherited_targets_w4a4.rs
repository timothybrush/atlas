...
//! The `AVAROK_NO_WARP_BLOCKSCALE_MMA` opt-out on the targets that inherit
...
const GUARD_FLAG: &str = "-DAVAROK_NO_WARP_BLOCKSCALE_MMA";
...
fn the_declared_absences_are_the_entry_points_the_define_removes() {
    ...
    let (_, guarded) = text
        .split_once("#ifndef AVAROK_NO_WARP_BLOCKSCALE_MMA")
...
fn both_inherited_targets_declare_the_w4a4_kernels_expected_absent() {
    for t in INHERITED {
        ...
        for kernel in GUARDED_KERNELS {
            ...
            assert!(
                reason.contains("AVAROK_NO_WARP_BLOCKSCALE_MMA"),
