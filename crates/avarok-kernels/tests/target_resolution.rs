...
//! GPU-free `AVAROK_SKIP_BUILD=1` runner where `all_ptx_sets()` is an empty
//! stub.
//!
...
use avarok_kernels::resolve::{ResolveCandidate, TargetResolveError, resolve_target};
use avarok_kernels::{ModelTypeMatch, resolve::resolve_pinned};
...
/// (`AVAROK_SKIP_BUILD=1`): every set of differently-named targets sharing a
