...
//! same idiom used by `avarok-closure` and `avarok-plugin::gate`.

use super::*;

#[test]
#[ignore = "requires nvcc and AVAROK_SKIP_BUILD unset"]
...
// CI host runs with `AVAROK_SKIP_BUILD=1`, which emits an empty stub
...
// machine via `cargo test -p avarok-kernels -- --ignored` after a
// real PTX build.
...
#[test]
#[ignore = "requires nvcc and AVAROK_SKIP_BUILD unset (multi-target build)"]
