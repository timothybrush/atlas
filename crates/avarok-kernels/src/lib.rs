...
//! When `AVAROK_TARGET_MODEL=*` or `AVAROK_TARGET_QUANT=*`, multiple
...
// `AVAROK_SKIP_BUILD=1` stub emits an empty `all_ptx_sets()` and names no
...
// stale and `AVAROK_KERNEL_SET_HASH` below already covers it.
mod target_defaults;
...
// this file (or `cargo clean -p avarok-kernels`) forces a fresh recompile
...
pub const KERNEL_SET_HASH: &str = env!("AVAROK_KERNEL_SET_HASH");
...
///
/// `{}` when the build compiled nothing (`AVAROK_SKIP_BUILD=1`) or could not
...
pub const TARGET_CLOSURES: &str = match option_env!("AVAROK_TARGET_CLOSURES") {
    Some(json) => json,
    None => "{}",
};
...
pub struct ModelBehavior {
    ...
    /// especially mid-tool-call — arXiv:2603.27905 AVAROK-RTC). Set `false`
    ...
    /// path that's strictly better than the previous "`[avarok]` Tool call
