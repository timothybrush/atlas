...
/// kernels tree — avarok-core is a leaf crate with no build script and no TOML
...
/// `avarok-kernels/tests/target_hints.rs` closes it: it reads every
...
impl std::fmt::Display for ArchMismatch {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let fix = match target_hint(self.device_cc) {
            Some(hw) => format!(
                "rebuild with AVAROK_TARGET_HW={hw} (kernels/{hw}/HARDWARE.toml arch must match \
