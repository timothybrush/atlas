...
//! `avarok-quant/src/fp8.rs` (unreachable, zero dependents) — with the
//! byte-exactness tests attached to the copy that never ran. Both crates
//! already depended on `avarok-core`, so the fix was to move the arithmetic
...
///
/// `AVAROK_DISABLE_RNE` is a bisect escape hatch that reverts to
...
pub fn f32_to_bf16(val: f32) -> u16 {
    ...
    if *DISABLE_RNE.get_or_init(|| std::env::var("AVAROK_DISABLE_RNE").is_ok()) {
...
mod tests {
    ...
    fn disable_rne_presence_uses_truncation() {
        ...
        const CHILD_MARKER: &str = "AVAROK_NUMERIC_RNE_CHILD";
        ...
        for value in ["0", "1"] {
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env(CHILD_MARKER, "1")
                .env("AVAROK_DISABLE_RNE", value)
                .output()
                .unwrap();
            assert!(
                output.status.success(),
                "AVAROK_DISABLE_RNE={value} child failed:\n{}",
