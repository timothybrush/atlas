...
impl ModelConfig {
    ...
    /// FP32 lm_head path (gated by `AVAROK_GEMMA4_FP32_LMHEAD=1`) can
    ...
    pub fn skip_lm_head_quantization(&self) -> bool {
        ...
        // precision. (Replaces the former AVAROK_LMHEAD_BF16 env var; PCND: explicit arg.)
        ...
        if self.model_type == "gemma4" && self.num_experts == 0 {
            ...
            return std::env::var("AVAROK_GEMMA4_LMHEAD_NVFP4").ok().as_deref() != Some("1");
        }
        false
    }
