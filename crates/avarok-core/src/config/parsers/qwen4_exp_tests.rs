...
///
///     AVAROK_QWEN4_EXP_CONFIG=/path/to/snapshot/config.json \
///       cargo test -p avarok-core --lib qwen4_exp
#[test]
fn parses_the_real_config_json_when_present() {
    let Ok(path) = std::env::var("AVAROK_QWEN4_EXP_CONFIG") else {
        eprintln!("AVAROK_QWEN4_EXP_CONFIG unset — skipping real-checkpoint parse");
        return;
    };
