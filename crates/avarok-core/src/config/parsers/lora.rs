...
pub fn allow_partial_targets() -> bool {
    ...
    *V.get_or_init(|| {
        std::env::var("AVAROK_LORA_ALLOW_PARTIAL")
...
fn validate_target_module(entry: &str) -> Result<()> {
    ...
    match leaf {
            ...
            if !allow_partial_targets() =>
        {
            bail!(
                "REJECT(gdn): target module '{leaf}' is a GDN/linear-attention projection; GDN \
                 ...
                 AVAROK_LORA_ALLOW_PARTIAL=1 to load the adapter anyway, applying only its \
        ...
        // (`classify_key`) + `AVAROK_LORA_EXPERTS` are the authority on whether the
