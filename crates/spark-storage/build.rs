...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=ATLAS_PREDICTOR_ARCH");
...
/// `ATLAS_PREDICTOR_ARCH` still wins when set explicitly, and the literal
/// survives as the announced fallback for an unreadable file.
fn resolve_predictor_arch() -> String {
    const FALLBACK: &str = "sm_121";
    if let Ok(explicit) = std::env::var("ATLAS_PREDICTOR_ARCH") {
        return explicit;
    }
        ...
        None => {
            println!(
                "cargo:warning=spark-storage: no [hardware].arch in {} — predictor kernels fall \
                 ...
                 ATLAS_PREDICTOR_ARCH to override",
