...
//! under `target/avarok-tier-tests`, with an EINVAL-skip when the filesystem
//! (tmpfs/overlay) refuses O_DIRECT so containerized CI doesn't break.

use std::path::Path;

use avarok_tier::{DirectSwapFile, Residency, SwapStore, VecSlotArena};
...
fn o_direct_file(record_bytes: usize, tag: &str) -> Option<(DirectSwapFile, std::path::PathBuf)> {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../target/avarok-tier-tests");
    ...
    match DirectSwapFile::create(&path, record_bytes) {
        ...
        Err(e) => {
            ...
            if std::env::var_os("AVAROK_TIER_REQUIRE_O_DIRECT").is_some() || !unsupported {
                panic!(
