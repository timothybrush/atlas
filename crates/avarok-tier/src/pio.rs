...
//! time (`avarok-tier::direct_swap`, `spark-model`'s snapshot arena, and
//! `spark-storage`'s file backend). `avarok-tier` is the crate all three
//! already depend on.
//!
...
mod tests {
    ...
    fn a_block_read_past_eof_succeeds_for_the_bytes_that_exist() {
        let dir = std::env::temp_dir().join(format!("avarok_pio_tail_{}", std::process::id()));
    ...
    fn round_trip_at_offset() {
        ...
        let dir = std::env::temp_dir().join(format!("avarok_pio_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
