...
use avarok_plugin::benchmarks::serve_matrix::ServeMatrix;
use avarok_plugin::benchmarks::serve_matrix::host::{ServeCandidate, ServeHost, ServeOptions};
use avarok_plugin::{
...
fn store(name: &str) -> ArtifactStore {
    let dir = std::env::temp_dir().join(format!("avarok-sm-{name}-{}", std::process::id()));
