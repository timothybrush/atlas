...
//!
//! ```text
//! HOME=/path/to/writable cargo test -p avarok-plugin --test kat_equality_e2e -- --ignored
//! ```
...
use avarok_plugin::{ArtifactStore, ParamValues, TargetEndpoint, VerdictKind};
...
async fn run_against(port: u16) -> avarok_plugin::RunRecord {
    run_with_orders(port, "2").await
}

async fn run_with_orders(port: u16, orders: &str) -> avarok_plugin::RunRecord {
    ...
    let descriptor = avarok_plugin::registry::find("kat-equality-gate").expect("registered");
    ...
    tokio::task::spawn_blocking(move || {
        avarok_plugin::headless::run_blocking(
            ...
            avarok_plugin::headless::RunRequest {
                ...
                values,
                target,
                options: avarok_plugin::headless::HeadlessOptions::cli("test"),
            },
