...
use avarok_plugin::coherence::{self, CoherencePolicy};
use avarok_plugin::plugin::TargetEndpoint;
...
impl avarok_plugin::headless::RunReporter for WarningReporter {
    fn event(&mut self, event: &avarok_plugin::PluginEvent) {
        if let avarok_plugin::PluginEvent::Log(line) = event
            && line.level == avarok_plugin::LogLevel::Warn
        {
...
async fn a_failed_probe_warns_but_still_runs_the_benchmark() {
    use avarok_plugin::headless::{HeadlessOptions, RunRequest, run_blocking};
    use avarok_plugin::{ArtifactStore, BenchmarkExecutor, ParamValues, registry};
...
async fn the_model_list_survives_chunked_framing() {
    let mock =
        mock_endpoint::start_saying(Some("4 Paris".into()), 1, Duration::ZERO, Duration::ZERO)
            .await;
    let models = avarok_plugin::http::list_models(&target(mock.port), Duration::from_secs(5))
        .await
