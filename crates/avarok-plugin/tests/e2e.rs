...
use avarok_plugin::benchmarks::{concurrency::ConcurrencySweep, ttft::TtftGate};
use avarok_plugin::{
...
fn temp_store(name: &str) -> ArtifactStore {
    let dir = std::env::temp_dir().join(format!("avarok-e2e-{name}-{}", std::process::id()));
...
async fn the_warm_gate_records_a_baseline_then_gates_against_it() {
    ...
    let mut bench = TtftGate::new(avarok_plugin::benchmarks::ttft::Mode::Warm);
    bench.load(h).await.expect("load");
    let mut values = ParamValues::defaults(&bench.parameters());
    ...
    let mut second = TtftGate::new(avarok_plugin::benchmarks::ttft::Mode::Warm);
...
async fn the_cold_gate_issues_one_request_per_sample_without_priming() {
    ...
    let mut bench = TtftGate::new(avarok_plugin::benchmarks::ttft::Mode::Cold);
    bench.load(h).await.expect("load");
    let mut values = ParamValues::defaults(&bench.parameters());
...
async fn a_headless_run_persists_a_record_the_history_pane_can_read() {
    ...
    let executor =
        avarok_plugin::BenchmarkExecutor::new(tokio::runtime::Handle::current(), store.clone());
    ...
    let outcome = tokio::task::spawn_blocking(move || {
        avarok_plugin::headless::run_blocking(
            &executor,
            avarok_plugin::headless::RunRequest {
                descriptor,
                values,
                target,
                options: avarok_plugin::headless::HeadlessOptions::cli("1.0.0-beta-preview"),
            },
            &mut avarok_plugin::headless::SilentReporter,
            &|| false,
        )
    })
    .await
    ...
    let back = avarok_plugin::history::load(&store, "concurrency-sweep");
