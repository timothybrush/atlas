...
fn adding_a_changed_file_never_removes_a_required_gate() {
    let base = ["crates/avarok-plugin/src/benchmarks/bfcl/report.rs"];
    let additions = [
...
/// `AVAROK_GDN_LIB`, and a committed recipe fixture points that at
...
/// `crates/avarok-plugin/src/gate`, which contains `coverage.rs`. That overlap
...
fn the_flat_concurrency_driver_invalidates_only_its_own_gate() {
    ...
    for path in [
        ...
        "crates/avarok-plugin/src/benchmarks/stats.rs",
        "crates/avarok-plugin/src/benchmarks/transcript.rs",
        "crates/avarok-plugin/src/benchmarks/concurrency.rs.bak",
...
fn a_change_to_the_equality_driver_reopens_that_gate_and_no_other() {
    let hit =
        coverage::invalidated_by(["crates/avarok-plugin/src/benchmarks/kat_equality/compare.rs"]);
    assert_eq!(
        hit,
        ["kat-equality-gate"],
