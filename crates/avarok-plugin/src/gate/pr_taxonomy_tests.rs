...
fn a_replayed_ledger_event_collapses_on_read() {
    ...
    let root = dir.path();
    let path = avarok_governance::ledger::path_for(root, 433);
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    let ev = |run: &str, at: u64, value: &str| avarok_governance::event::Event {
        pr: 433,
        head_sha: "abc1234567".into(),
        run_id: run.into(),
        attempt: 1,
        at,
        kind: avarok_governance::event::EventKind::Category {
            value: value.into(),
            status: "ok".into(),
        },
    };
    for e in [
        ...
        avarok_governance::ledger::append(&path, &e).unwrap();
    }
    let raw = avarok_governance::ledger::read_all(&path).unwrap();
    ...
    assert_eq!(
        deduped
            ...
            .iter()
            .map(|event| {
                let avarok_governance::event::EventKind::Category { value, status } = &event.kind
                else {
