...
//! measured.
//!
//! A run stored only in `~/.avarok` answers for one box and one person. A gate
...
///   (`crates/avarok-plugin/assets/bfcl/*.py`), which define the score itself.
...
/// The short commit id for this working tree. `AVAROK_GATE_SHA` overrides —
/// the escape hatch for a checkout without git metadata.
pub fn git_sha(root: &Path) -> Result<String> {
    if let Some(explicit) = std::env::var_os("AVAROK_GATE_SHA") {
        let sha = explicit.to_string_lossy().trim().to_string();
        if sha.is_empty() {
            bail!("AVAROK_GATE_SHA is set but empty");
        }
        return Ok(sha);
    }
    let out = std::process::Command::new("git")
    ...
    if !out.status.success() {
        bail!(
            "git rev-parse failed — {} is not a git checkout (or git is \
             missing); set AVAROK_GATE_SHA to record a gate run",
            root.display()
        );
    }
    let sha = String::from_utf8_lossy(&out.stdout).trim().to_string();
