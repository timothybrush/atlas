...
fn repo_root() -> PathBuf {
    // crates/avarok-plugin -> repo root
...
fn tmp(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("avarok-taxon-{}-{name}", std::process::id()));
...
fn paths_map_to_the_right_nodes() {
    ...
    assert_eq!(hardware_of("crates/avarok-plugin/src/lib.rs"), None);
...
fn a_non_kernel_change_affects_no_target_here() {
    ...
    assert!(
        affected(
            &root,
            &["crates/avarok-plugin/src/gate/check.rs".to_string()]
        )
        .is_empty(),
