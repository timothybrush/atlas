...
/// `crates/avarok-plugin/src/gate` prefix, and `BOUNDARY_FILES` listed only
...
/// unrelated `avarok-kernels` file as invalidating; it read red by accident,
/// not by rule.
///
...
fn gate_sources_are_all_classified() {
    ...
    for entry in std::fs::read_dir(&gate_dir).expect("gate dir listable") {
        ...
        on_disk.insert(format!("crates/avarok-plugin/src/gate/{name}"));
    }

    let boundary: std::collections::BTreeSet<&str> = super::coverage::BOUNDARY_FILES
        .iter()
        .copied()
        .filter(|p| p.starts_with("crates/avarok-plugin/src/gate/"))
        .collect();
