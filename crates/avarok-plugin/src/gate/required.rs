...
pub fn intent_source(root: &std::path::Path, pr: Option<u64>) -> IntentSource {
    ...
    let ledger = avarok_governance::ledger::path_for(root, pr);
    if !ledger.exists() {
        return IntentSource::NotRecorded { ledger };
    }
    let journey = match avarok_governance::ledger::read_all(&ledger) {
        Ok(j) => j.deduplicated(),
    ...
    for event in &journey.events {
        let avarok_governance::event::EventKind::Category { value, status } = &event.kind else {
            continue;
        };
