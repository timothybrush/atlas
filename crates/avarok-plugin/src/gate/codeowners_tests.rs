...
fn multiple_stars_within_one_segment_are_matched() {
    ...
    assert_eq!(owners_of(&rules, "docs/avarok-bench-check.md"), ["@a"]);
    assert!(owners_of(&rules, "docs/avarok-bench-check.txt").is_empty());
...
fn a_missing_codeowners_file_yields_no_rules_rather_than_failing() {
    let empty = std::env::temp_dir().join(format!("avarok-noowners-{}", std::process::id()));
