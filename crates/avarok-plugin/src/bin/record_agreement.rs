...
use avarok_plugin::gate::Standing;
use avarok_plugin::gate::agreement::{AddedRecord, check, standing_at};
...
fn main() -> std::process::ExitCode {
    ...
    let root = match std::env::current_dir()
        .ok()
        .and_then(|d| avarok_plugin::gate::git_rev_parse_toplevel(&d).ok())
    {
        Some(r) => r,
        None => {
    ...
    let head = match std::env::var("RECORD_AGREEMENT_HEAD")
        .ok()
        .or_else(|| avarok_plugin::gate::git_sha(&root).ok())
    {
        Some(h) => h,
        None => {
    ...
    for a in &args {
        ...
        let parsed = avarok_plugin::gate::read_record(path).ok();
        let hardware = parsed
            .as_ref()
            .map(avarok_plugin::hardware::equivalence::HardwareFingerprint::from_record);
