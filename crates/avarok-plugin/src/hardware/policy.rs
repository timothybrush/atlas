...
//! explicit `AVAROK_HW_TEMP_GATE=1` to become gating. Everything they see is
...
pub struct PolicyOptions {
    /// `AVAROK_NO_HW_PRECHECK=1` — never REFUSE. Everything is still collected,
    ...
    /// `AVAROK_HW_TEMP_GATE=1` — promote the absolute-temperature ceilings from
...
pub const KILL_SWITCH_ENV: &str = "AVAROK_NO_HW_PRECHECK";
/// Env var that opts in to the absolute-temperature ceilings.
pub const TEMP_GATE_ENV: &str = "AVAROK_HW_TEMP_GATE";
...
/// a third box exists, which is why they gate only under `AVAROK_HW_TEMP_GATE`
...
pub fn precheck(
    ...
    // Recorded by default; gating only under AVAROK_HW_TEMP_GATE=1.
