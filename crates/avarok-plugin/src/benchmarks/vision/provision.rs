...
//! `~/.avarok/artifacts/vision` — the fixture images, laid down on `load()`.
//!
//! Follows the BFCL provisioning pattern (`benchmarks/bfcl/provision.rs`):
//! assets are compiled into the binary from `crates/avarok-plugin/assets/`,
...
///
/// ★ AVAROK APPLIES THE TAG, so the pair must DISAGREE: rotating 90 CW carries
...
/// `~/.avarok/artifacts/vision`, matching BFCL's `~/.avarok/artifacts/bfcl`.
