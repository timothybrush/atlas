...
fn the_default_is_the_harness_ssot_path() {
    ...
    // all default to ${HOME}/.cargo/avarok-warm-target. Pointing anywhere else is
    ...
    let d = dir_from(None, Some("/home/x".into()), "avarok-warm-target").unwrap();
    assert_eq!(d, std::path::Path::new("/home/x/.cargo/avarok-warm-target"));
    let t = dir_from(None, Some("/home/x".into()), "avarok-warm-template").unwrap();
    assert_eq!(
        t,
        std::path::Path::new("/home/x/.cargo/avarok-warm-template")
    );
    ...
    assert!(scorer.contains("\".cargo\", \"avarok-warm-target\""));
}
...
fn the_template_covers_the_feature_superset_and_every_touched_crate() {
    ...
    assert!(TEMPLATE_MAIN.contains("AVAROK_HARNESS_PORT"));
}

#[test]
fn writing_the_template_is_idempotent_so_cargo_does_not_rebuild_it() {
    let dir = std::env::temp_dir().join(format!("avarok-warm-tpl-{}", std::process::id()));
