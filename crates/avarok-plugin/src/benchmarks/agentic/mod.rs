...
AVAROK_HARNESS_PORT env var (default 3001) — use `let port: u16 = \
std::env::var(\"AVAROK_HARNESS_PORT\").unwrap_or_else(|_| \"3001\".to_string()).parse().unwrap();` \
...
fuser -k ${AVAROK_HARNESS_PORT:-3001}/tcp 2>/dev/null || true`.";
