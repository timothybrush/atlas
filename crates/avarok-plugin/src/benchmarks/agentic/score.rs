...
pub async fn webserver_test(
    ...
    build
        ...
        .env("AVAROK_HARNESS_PORT", port.to_string())
        // The harness's cargo shim detaches `cargo run` only when
        // AVAROK_AGENT_SHELL is set, and its own comment reserves that for the
        // agent: "The SCORER (no AVAROK_AGENT_SHELL) + build/test pass through."
        // Inheriting it here would detach the server we are about to supervise.
        .env_remove("AVAROK_AGENT_SHELL")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .kill_on_drop(true);
...
/// ~/avarok/tests/foo.rs tests/foo.rs` would otherwise credit `wrote_tests` to an
