...
fn sandbox(name: &str) -> std::path::PathBuf {
    let d = std::env::temp_dir().join(format!("avarok-agentic-{name}-{}", std::process::id()));
...
fn the_evidence_walk_neither_follows_a_symlink_nor_counts_one() {
    ...
    std::fs::write(d.join("Cargo.toml"), "[package]").unwrap();
    let elsewhere = d.parent().unwrap().join("avarok-agentic-symlinks-donor");
...
async fn the_ephemeral_port_stays_reserved_while_the_project_builds() {
    ...
    std::fs::write(
        d.join("build.rs"),
        r#"fn main() {
    let port: u16 = std::env::var("AVAROK_HARNESS_PORT").unwrap().parse().unwrap();
    assert!(
...
async fn a_missing_src_dir_fails_fast_without_building() {
    ...
    let d = std::env::temp_dir().join(format!("avarok-agentic-nosrc-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&d);
...
fn a_bind_failure_is_named_rather_than_reported_as_silence() {
    ...
    let p = std::env::temp_dir().join(format!("avarok-ws-err-{}.log", std::process::id()));
    std::fs::write(
        &p,
