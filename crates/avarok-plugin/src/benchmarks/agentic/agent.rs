...
/// harness pins beside it (`AVAROK_OPENCODE_OUTPUT_CAP` → `limit.output`, which
...
/// `avarok*` model, and that is right for a research harness: it samples the
...
/// `~/.config/opencode/agents/avarok.md` — the agent `run_tier.sh` selects with
/// `default_agent: avarok`. `LLMRequestPrep.prepare` uses an agent's own prompt
...
fn request_body(model: &str, messages: &[Value], tools: &Value, max_tokens: usize) -> Value {
    ...
    // AVAROK_AGENTIC_SAMPLING=model-card: omit the greedy pins so the server's
    // card presets own sampling (A/B arm; the GATE default stays pinned-greedy
    // per the rationale above TEMPERATURE).
    if std::env::var("AVAROK_AGENTIC_SAMPLING").as_deref() != Ok("model-card") {
