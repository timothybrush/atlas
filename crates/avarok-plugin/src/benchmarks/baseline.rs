...
//! Per-benchmark baselines, stored beside the runs in `~/.avarok`.
//!
...
//!
//! Baselines are **box-local and config-local by construction** (`~/.avarok` is
...
mod tests {
    ...
    fn store(name: &str) -> ArtifactStore {
        let d = std::env::temp_dir().join(format!("avarok-baseline-{name}-{}", std::process::id()));
