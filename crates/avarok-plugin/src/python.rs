...
///
/// `AVAROK_PYTHON` overrides the search — the box may keep the usable
...
pub async fn find_python(min_major: u32, min_minor: u32) -> Result<PathBuf> {
    let candidates: Vec<PathBuf> = match std::env::var_os("AVAROK_PYTHON") {
    ...
    bail!(
        "no usable python found (need >= {min_major}.{min_minor}). Tried:\n  {}\n\
         Install python3 or point AVAROK_PYTHON at an interpreter.",
        tried.join("\n  ")
    )
}
...
pub async fn pip_install(interpreter: &Path, requirements: &Path) -> Result<()> {
    ...
    .context(
        "pip install failed — this step needs network access to PyPI \
         (air-gapped boxes must pre-populate ~/.avarok/artifacts)",
    )?;
    Ok(())
}
...
mod tests {
    ...
    async fn a_successful_command_captures_both_streams_and_honors_cwd() {
        let dir = std::env::temp_dir().join(format!("avarok-python-run-{}", std::process::id()));
