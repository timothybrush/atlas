...
//! hands `avarok_kernels::metallib_modules()` to `MetalGpuBackend::new`. That
...
//!
//! That is exactly what shipped. avarok-kernels' `build.rs` honours
//! `AVAROK_SKIP_BUILD` before anything else and emits a stub whose
//! `metallib_modules()` is `Vec::new()`; `ci.yml` sets `AVAROK_SKIP_BUILD: "1"`
...
fn embedded_metallib_set_is_not_empty() {
    ...
    assert!(
        !modules.is_empty(),
        "avarok_kernels::metallib_modules() is empty -- no Metal kernels were \
         ...
         report on one.\n\
         \n\
         Cause: avarok-kernels/build.rs took its skip branch. Either \
         AVAROK_SKIP_BUILD is set to 1/true, or the build is on macOS with no \
         AVAROK_TARGET_HW set (the auto-skip).\n\
         \n\
         Build the kernels instead of skipping them:\n\
         \x20 AVAROK_SKIP_BUILD=0 AVAROK_TARGET_HW=metal \\\n\
         \x20 AVAROK_TARGET_MODEL=qwen3-5-4b-vlm-mlx-int8 AVAROK_TARGET_QUANT=mlx_int8 \\\n\
