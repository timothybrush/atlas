...
fn carried_state_is_the_same_allocation_not_an_equal_one() {
    let first = Carried::from_env().expect("no AVAROK_* config is set in the test environment");
...
fn building_from_env_twice_would_lose_the_stores() {
    let first = Carried::from_env().expect("no AVAROK_* config is set in the test environment");
    let second = Carried::from_env().expect("no AVAROK_* config is set in the test environment");
    assert!(
...
fn carried_uses_the_process_limiter_rather_than_minting_its_own() {
    ...
    let carried = Carried::from_env().expect("no AVAROK_* config is set in the test environment");
